/* GENERATED by scripts/generate.mjs from icons/outline/arrows/import-circle.svg, icons/solid/arrows/import-circle.svg. Do not edit by hand. */
import { IconBase } from "../../icon-base.js";
import type { QeetrixIconProps } from "../../types.js";

export function ImportCircle({ variant = "outline", ...props }: QeetrixIconProps & { variant?: "outline" | "solid" }) {
  if (variant === "solid") {
    return (
      <IconBase variant={variant} {...props}>
        <path
          d="M21.14 9.90039H12.68V13.5704L14.25 12.0004C14.54 11.7104 15.02 11.7104 15.31 12.0004C15.6 12.2904 15.6 12.7704 15.31 13.0604L12.46 15.9004C12.17 16.1904 11.69 16.1904 11.4 15.9004L8.55 13.0604C8.4 12.9104 8.33 12.7204 8.33 12.5304C8.33 12.3404 8.41 12.1504 8.56 12.0004C8.85 11.7104 9.33 11.7104 9.62 12.0004L11.18 13.5604V9.90039H2.86C2.38 9.90039 2 10.2804 2 10.7604C2 16.6504 6.11 20.7604 12 20.7604C17.89 20.7604 22 16.6504 22 10.7604C22 10.2804 21.62 9.90039 21.14 9.90039Z"
          fill="currentColor"
        />
        <path
          d="M12.6799 3.99023C12.6799 3.58023 12.3399 3.24023 11.9299 3.24023C11.5199 3.24023 11.1799 3.58023 11.1799 3.99023V9.89023H12.6799V3.99023Z"
          fill="currentColor"
        />
      </IconBase>
    );
  }
  return (
    <IconBase variant={variant} {...props}>
      <path
        d="M11.88 14.9901C11.69 14.9901 11.5 14.9201 11.35 14.7701L8.79001 12.2101C8.50001 11.9201 8.50001 11.4401 8.79001 11.1501C9.08001 10.8601 9.56001 10.8601 9.85001 11.1501L11.88 13.1801L13.91 11.1501C14.2 10.8601 14.68 10.8601 14.97 11.1501C15.26 11.4401 15.26 11.9201 14.97 12.2101L12.41 14.7701C12.26 14.9201 12.07 14.9901 11.88 14.9901Z"
        fill="currentColor"
      />
      <path
        d="M11.8799 14.92C11.4699 14.92 11.1299 14.58 11.1299 14.17V4C11.1299 3.59 11.4699 3.25 11.8799 3.25C12.2899 3.25 12.6299 3.59 12.6299 4V14.17C12.6299 14.58 12.2899 14.92 11.8799 14.92Z"
        fill="currentColor"
      />
      <path
        d="M12 20.9297C6.85 20.9297 3.25 17.3297 3.25 12.1797C3.25 11.7697 3.59 11.4297 4 11.4297C4.41 11.4297 4.75 11.7697 4.75 12.1797C4.75 16.4497 7.73 19.4297 12 19.4297C16.27 19.4297 19.25 16.4497 19.25 12.1797C19.25 11.7697 19.59 11.4297 20 11.4297C20.41 11.4297 20.75 11.7697 20.75 12.1797C20.75 17.3297 17.15 20.9297 12 20.9297Z"
        fill="currentColor"
      />
    </IconBase>
  );
}
