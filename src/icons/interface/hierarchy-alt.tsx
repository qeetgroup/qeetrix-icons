/* GENERATED by scripts/generate.mjs from icons/outline/interface/hierarchy-alt.svg, icons/solid/interface/hierarchy-alt.svg. Do not edit by hand. */
import { IconBase } from "../../icon-base.js";
import type { QeetrixIconProps } from "../../types.js";

export function HierarchyAlt({ variant = "outline", ...props }: QeetrixIconProps & { variant?: "outline" | "solid" }) {
  if (variant === "solid") {
    return (
      <IconBase variant={variant} {...props}>
        <path
          d="M18.75 22C20.5449 22 22 20.5449 22 18.75C22 16.9551 20.5449 15.5 18.75 15.5C16.9551 15.5 15.5 16.9551 15.5 18.75C15.5 20.5449 16.9551 22 18.75 22Z"
          fill="currentColor"
        />
        <path
          d="M19 8C20.6569 8 22 6.65685 22 5C22 3.34315 20.6569 2 19 2C17.3431 2 16 3.34315 16 5C16 6.65685 17.3431 8 19 8Z"
          fill="currentColor"
        />
        <path
          d="M5 8C6.65685 8 8 6.65685 8 5C8 3.34315 6.65685 2 5 2C3.34315 2 2 3.34315 2 5C2 6.65685 3.34315 8 5 8Z"
          fill="currentColor"
        />
        <path
          d="M4.83 8.02C5.8 10.8 8.42 12.67 11.37 12.67C11.38 12.67 11.39 12.67 11.4 12.67L14.93 12.66C16.45 12.64 17.81 13.67 18.22 15.15V17C18.22 17.42 18.56 17.76 18.99 17.76C19.41 17.76 19.75 17.42 19.75 17V5.76C19.75 5.34 19.41 5 18.99 5C18.57 5 18.22 5.34 18.22 5.76V12.38C17.34 11.6 16.19 11.12 14.94 11.12C14.93 11.12 14.93 11.12 14.92 11.12L11.39 11.13C11.38 11.13 11.38 11.13 11.37 11.13C9.08 11.13 7.03 9.68 6.28 7.51C6.16 7.2 5.87 7 5.55 7C5.47 7 5.38 7.02 5.3 7.04C4.9 7.18 4.69 7.62 4.83 8.02Z"
          fill="currentColor"
        />
      </IconBase>
    );
  }
  return (
    <IconBase variant={variant} {...props}>
      <path
        d="M5 15.75C4.59 15.75 4.25 15.41 4.25 15V8C4.25 7.59 4.59 7.25 5 7.25C5.41 7.25 5.75 7.59 5.75 8V15C5.75 15.41 5.41 15.75 5 15.75Z"
        fill="currentColor"
      />
      <path
        d="M5.25 22.75C3.04 22.75 1.25 20.96 1.25 18.75C1.25 16.54 3.04 14.75 5.25 14.75C7.46 14.75 9.25 16.54 9.25 18.75C9.25 20.96 7.46 22.75 5.25 22.75ZM5.25 16.25C3.87 16.25 2.75 17.37 2.75 18.75C2.75 20.13 3.87 21.25 5.25 21.25C6.63 21.25 7.75 20.13 7.75 18.75C7.75 17.37 6.63 16.25 5.25 16.25Z"
        fill="currentColor"
      />
      <path
        d="M5 8.75C2.93 8.75 1.25 7.07 1.25 5C1.25 2.93 2.93 1.25 5 1.25C7.07 1.25 8.75 2.93 8.75 5C8.75 7.07 7.07 8.75 5 8.75ZM5 2.75C3.76 2.75 2.75 3.76 2.75 5C2.75 6.24 3.76 7.25 5 7.25C6.24 7.25 7.25 6.24 7.25 5C7.25 3.76 6.24 2.75 5 2.75Z"
        fill="currentColor"
      />
      <path
        d="M19 8.75C16.93 8.75 15.25 7.07 15.25 5C15.25 2.93 16.93 1.25 19 1.25C21.07 1.25 22.75 2.93 22.75 5C22.75 7.07 21.07 8.75 19 8.75ZM19 2.75C17.76 2.75 16.75 3.76 16.75 5C16.75 6.24 17.76 7.25 19 7.25C20.24 7.25 21.25 6.24 21.25 5C21.25 3.76 20.24 2.75 19 2.75Z"
        fill="currentColor"
      />
      <path
        d="M5.13011 15.7503C5.07011 15.7503 5.01011 15.7403 4.94011 15.7303C4.54011 15.6303 4.30011 15.2203 4.40011 14.8203C4.95011 12.7003 6.86011 11.2203 9.05011 11.2203C9.06011 11.2203 9.06011 11.2203 9.07011 11.2203L12.5001 11.2303C12.5101 11.2303 12.5101 11.2303 12.5201 11.2303C14.7401 11.2303 16.7301 9.82026 17.4601 7.72026C17.6001 7.33026 18.0201 7.12027 18.4101 7.26027C18.8001 7.40027 19.0101 7.82025 18.8701 8.21025C17.9301 10.9202 15.3401 12.7003 12.4901 12.7303L9.06011 12.7203H9.05011C7.54011 12.7203 6.23011 13.7403 5.85011 15.2003C5.77011 15.5303 5.46011 15.7503 5.13011 15.7503Z"
        fill="currentColor"
      />
    </IconBase>
  );
}
