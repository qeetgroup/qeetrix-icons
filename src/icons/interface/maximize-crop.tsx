/* GENERATED from icons/outline/interface/maximize-crop.svg, icons/solid/interface/maximize-crop.svg by scripts/generate.mjs. Do not edit by hand. */
import type { SVGProps } from "react";

export function MaximizeCrop({ variant = "outline", ...props }: SVGProps<SVGSVGElement> & { variant?: "outline" | "solid" }) {
  if (variant === "solid") {
    return (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
        <g clipPath="url(#clip0_4418_8415)">
          <path d="M16.19 2H7.81C4.17 2 2 4.17 2 7.81V16.18C2 19.83 4.17 22 7.81 22H16.18C19.82 22 21.99 19.83 21.99 16.19V7.81C22 4.17 19.83 2 16.19 2ZM11.53 13.53L7.81 17.25H10C10.41 17.25 10.75 17.59 10.75 18C10.75 18.41 10.41 18.75 10 18.75H6C5.9 18.75 5.8 18.73 5.71 18.69C5.53 18.61 5.38 18.47 5.3 18.28C5.27 18.19 5.25 18.1 5.25 18V14C5.25 13.59 5.59 13.25 6 13.25C6.41 13.25 6.75 13.59 6.75 14V16.19L10.47 12.47C10.76 12.18 11.24 12.18 11.53 12.47C11.82 12.76 11.82 13.24 11.53 13.53ZM18.75 10C18.75 10.41 18.41 10.75 18 10.75C17.59 10.75 17.25 10.41 17.25 10V7.81L13.53 11.53C13.38 11.68 13.19 11.75 13 11.75C12.81 11.75 12.62 11.68 12.47 11.53C12.18 11.24 12.18 10.76 12.47 10.47L16.19 6.75H14C13.59 6.75 13.25 6.41 13.25 6C13.25 5.59 13.59 5.25 14 5.25H18C18.1 5.25 18.19 5.27 18.29 5.31C18.47 5.39 18.62 5.53 18.7 5.72C18.73 5.81 18.75 5.9 18.75 6V10Z" fill="white"/>
        </g>
        <defs>
          <clipPath id="clip0_4418_8415">
            <rect width="24" height="24" fill="white"/>
          </clipPath>
        </defs>
      </svg>
    );
  }
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
      <g clipPath="url(#clip0_4418_7196)">
        <path d="M21 9.75C20.59 9.75 20.25 9.41 20.25 9V3.75H15C14.59 3.75 14.25 3.41 14.25 3C14.25 2.59 14.59 2.25 15 2.25H21C21.41 2.25 21.75 2.59 21.75 3V9C21.75 9.41 21.41 9.75 21 9.75Z" fill="white"/>
        <path d="M9 21.75H3C2.59 21.75 2.25 21.41 2.25 21V15C2.25 14.59 2.59 14.25 3 14.25C3.41 14.25 3.75 14.59 3.75 15V20.25H9C9.41 20.25 9.75 20.59 9.75 21C9.75 21.41 9.41 21.75 9 21.75Z" fill="white"/>
        <path d="M13.4999 11.2495C13.3099 11.2495 13.1199 11.1795 12.9699 11.0295C12.6799 10.7395 12.6799 10.2595 12.9699 9.96945L20.4699 2.46945C20.7599 2.17945 21.2399 2.17945 21.5299 2.46945C21.8199 2.75945 21.8199 3.23945 21.5299 3.52945L14.0299 11.0295C13.8799 11.1795 13.6899 11.2495 13.4999 11.2495Z" fill="white"/>
        <path d="M2.99994 21.7495C2.80994 21.7495 2.61994 21.6795 2.46994 21.5295C2.17994 21.2395 2.17994 20.7595 2.46994 20.4695L9.96994 12.9695C10.2599 12.6795 10.7399 12.6795 11.0299 12.9695C11.3199 13.2595 11.3199 13.7395 11.0299 14.0295L3.52994 21.5295C3.37994 21.6795 3.18994 21.7495 2.99994 21.7495Z" fill="white"/>
      </g>
      <defs>
        <clipPath id="clip0_4418_7196">
          <rect width="24" height="24" fill="white"/>
        </clipPath>
      </defs>
    </svg>
  );
}
