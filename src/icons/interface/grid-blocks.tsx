/* GENERATED by scripts/generate.mjs from icons/outline/interface/grid-blocks.svg, icons/solid/interface/grid-blocks.svg. Do not edit by hand. */
import { IconBase } from "../../icon-base.js";
import type { QeetrixIconProps } from "../../types.js";

export function GridBlocks({ variant = "outline", ...props }: QeetrixIconProps & { variant?: "outline" | "solid" }) {
  if (variant === "solid") {
    return (
      <IconBase variant={variant} {...props}>
        <path
          d="M22 7.8093V11.2493H16.75V2.0293C20.05 2.2293 22 4.3593 22 7.8093Z"
          fill="currentColor"
        />
        <path d="M22 12.75V16.19C22 19.64 20.05 21.77 16.75 21.97V12.75H22Z" fill="currentColor" />
        <path d="M7.25 12.75V21.97C3.95 21.77 2 19.64 2 16.19V12.75H7.25Z" fill="currentColor" />
        <path
          d="M7.25 2.0293V11.2493H2V7.8093C2 4.3593 3.95 2.2293 7.25 2.0293Z"
          fill="currentColor"
        />
        <path d="M15.25 2H8.75V11.25H15.25V2Z" fill="currentColor" />
        <path d="M15.25 12.75H8.75V22H15.25V12.75Z" fill="currentColor" />
      </IconBase>
    );
  }
  return (
    <IconBase variant={variant} {...props}>
      <path
        d="M15 22.75H9C3.57 22.75 1.25 20.43 1.25 15V9C1.25 3.57 3.57 1.25 9 1.25H15C20.43 1.25 22.75 3.57 22.75 9V15C22.75 20.43 20.43 22.75 15 22.75ZM9 2.75C4.39 2.75 2.75 4.39 2.75 9V15C2.75 19.61 4.39 21.25 9 21.25H15C19.61 21.25 21.25 19.61 21.25 15V9C21.25 4.39 19.61 2.75 15 2.75H9Z"
        fill="currentColor"
      />
      <path
        d="M22 12.75H2C1.59 12.75 1.25 12.41 1.25 12C1.25 11.59 1.59 11.25 2 11.25H22C22.41 11.25 22.75 11.59 22.75 12C22.75 12.41 22.41 12.75 22 12.75Z"
        fill="currentColor"
      />
      <path
        d="M8.25 22.25C7.84 22.25 7.5 21.91 7.5 21.5V12C7.5 11.59 7.84 11.25 8.25 11.25C8.66 11.25 9 11.59 9 12V21.5C9 21.91 8.66 22.25 8.25 22.25Z"
        fill="currentColor"
      />
      <path
        d="M15.75 22.75C15.34 22.75 15 22.41 15 22V12C15 11.59 15.34 11.25 15.75 11.25C16.16 11.25 16.5 11.59 16.5 12V22C16.5 22.41 16.16 22.75 15.75 22.75Z"
        fill="currentColor"
      />
      <path
        d="M8.25 12.75C7.84 12.75 7.5 12.41 7.5 12V2C7.5 1.59 7.84 1.25 8.25 1.25C8.66 1.25 9 1.59 9 2V12C9 12.41 8.66 12.75 8.25 12.75Z"
        fill="currentColor"
      />
      <path
        d="M15.75 12.7491C15.34 12.7491 15 12.4091 15 11.9991V2.53906C15 2.12906 15.34 1.78906 15.75 1.78906C16.16 1.78906 16.5 2.12906 16.5 2.53906V11.9991C16.5 12.4091 16.16 12.7491 15.75 12.7491Z"
        fill="currentColor"
      />
    </IconBase>
  );
}
