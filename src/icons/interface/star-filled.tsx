/* GENERATED from icons/round-outline/interface/star-filled.svg, icons/round-solid/interface/star-filled.svg by scripts/generate.mjs. Do not edit by hand. */
import type { SVGProps } from "react";

export function StarFilled({ variant = "outline", shape: _shape = "round", ...props }: SVGProps<SVGSVGElement> & { variant?: "outline" | "solid"; shape?: "round" }) {
  if (variant === "solid") {
    return (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="white" {...props}>
        <g clipPath="url(#clip0_655_9444)">
          <path d="M18.0841 11.612C18.4509 11.6649 18.4509 12.3351 18.0841 12.388C14.1035 12.9624 12.9624 14.1035 12.388 18.0841C12.3351 18.4509 11.6649 18.4509 11.612 18.0841C11.0376 14.1035 9.89647 12.9624 5.91594 12.388C5.5491 12.3351 5.5491 11.6649 5.91594 11.612C9.89647 11.0376 11.0376 9.89647 11.612 5.91594C11.6649 5.5491 12.3351 5.5491 12.388 5.91594C12.9624 9.89647 14.1035 11.0376 18.0841 11.612Z" fill="currentColor"/>
        </g>
        <defs>
          <clipPath id="clip0_655_9444">
            <rect width="24" height="24" fill="white"/>
          </clipPath>
        </defs>
      </svg>
    );
  }
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="white" {...props}>
      <g clipPath="url(#clip0_655_7092)">
        <path d="M12 22.75C11.61 22.75 11.28 22.45 11.25 22.06C10.61 13.65 10.35 13.39 1.94 12.75C1.55 12.72 1.25 12.39 1.25 12C1.25 11.61 1.55 11.28 1.94 11.25C10.35 10.61 10.61 10.35 11.25 1.94C11.28 1.55 11.61 1.25 12 1.25C12.39 1.25 12.72 1.55 12.75 1.94C13.39 10.35 13.65 10.61 22.06 11.25C22.45 11.28 22.75 11.61 22.75 12C22.75 12.39 22.45 12.72 22.06 12.75C13.65 13.39 13.39 13.65 12.75 22.06C12.72 22.45 12.39 22.75 12 22.75ZM7.82 12C10.25 12.62 11.38 13.75 12 16.18C12.62 13.74 13.75 12.62 16.18 12C13.75 11.38 12.62 10.25 12 7.82C11.38 10.26 10.25 11.38 7.82 12Z" fill="currentColor"/>
      </g>
      <defs>
        <clipPath id="clip0_655_7092">
          <rect width="24" height="24" fill="white"/>
        </clipPath>
      </defs>
    </svg>
  );
}
